...
前端预览时实时脱敏（对应 F07 决议 Q16 选 C）。脱敏规则在共享模块 `resume-generator/scripts/resume-rules.mjs` —— 前端预览与后端 JSON 导出共用同一实现，避免行为分叉：
...
| 公司 | 整体替换 | 字节跳动 → [公司已隐藏] |
| 薪资 | 整体替换 | 30000 → [薪资已隐藏] |
| GitHub | 整体替换 | github.com/x → [GitHub已隐藏] |

用户在顶栏勾选脱敏开关，预览实时更新。导出（JSON/HTML/PDF）用同一份脱敏配置与规则。
